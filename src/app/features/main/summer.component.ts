...
export class SummerComponent implements OnInit, OnDestroy {
  ...
  /*   console.log("SummerComponent load colors", colors);
    const variables = [
  ...
  createNewItem(detail: ApartmentDetail | null, colors: Colors | null | undefined) {
    ...
    if (detail) {
      const item: Partial<ApartmentItem> = {apartmentDetailId: detail.id}
  ...
  updateDetail(header: Header | null, detail: ApartmentDetail | null) {
    ...
    console.log("updateDetail apartmentId", detail);

    const partial: Partial<ApartmentDetail> = {...detail, apartmentUrl: header?.apartmentUrl}
